...
class _EmptySearchStateState extends State<EmptySearchState>
  ...
  Widget _buildSearchSuggestions(bool isTr) {
    final suggestions = isTr
        ? ['Hidrojen', 'Oksijen', 'Altın', 'Demir', 'Karbon']
        : ['Hydrogen', 'Oxygen', 'Gold', 'Iron', 'Carbon'];
  ...
  Widget _buildHelpfulTips(bool isTr) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        gradient: LinearGradient(
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                isTr ? 'Arama İpuçları:' : 'Search Tips:',
                style: const TextStyle(
                  color: AppColors.white,
                  fontSize: 16,
                  fontWeight: FontWeight.w600,
          ...
          _buildTip(
            isTr
                ? '• Element adını, sembolünü veya atom numarasını yazın'
                : '• Type element name, symbol, or atomic number',
          ),
          _buildTip(
            isTr
          ...
          _buildTip(
            isTr
                ? '• Kısmi eşleşmeler de desteklenir'
